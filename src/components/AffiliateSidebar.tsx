...
export function AffiliateSidebar() {
  ...
  return (
    <Sidebar open={open} setOpen={setOpen} animate={false}>
      <SidebarBody className="justify-between gap-10 bg-[#1a1a2c]">
          ...
          <div className="flex flex-col gap-2">
            {bottomLinks.map((link, idx) => (
              <SidebarLink
                key={idx}
                link={link}
                className={`text-gray-400 hover:text-white hover:bg-gray-900/30 rounded-md transition-colors ${location.pathname === link.href ? "bg-gray-900/30 text-white" : ""
                  }`}
              />
            ))}
          </div>
